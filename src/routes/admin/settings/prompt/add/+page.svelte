<script>
	...
	import { validateAndFormat } from '../../../../../helpers/fields';
	import { asyncTryCatchToast } from '../../../../../lib/utils';
	import { getPromptFields } from '../../../../../helpers/fields';
	...
	const handleSubmit = async () => {
		...
		asyncTryCatchToast(
			async () => {
				await promptService.add({ ...data, created_at: new Date().toISOString() });
				prompts.set(await promptService.get());
			},
			{
				message: 'Prompt Added Successfully',
				redirect: '/admin/settings/prompt/browse'
			}
		);
	};
...
>
	<div class="flex flex-row gap-1 text-[14px] leading-5 font-medium items-center">
		<a href="/admin/setting/prompt/browse" class="text-gray-3">Prompt</a>
	...
	<div class:hidden={!cancel} class="fixed top-0 left-0 m-auto w-full h-screen bg-black-3 z-100">
		...
		>
			...
			<div class="flex flex-row gap-3 text-black font-semibold items-center w-full">
				...
				>
				<a
					href="/admin/settings/prompt/browse"
