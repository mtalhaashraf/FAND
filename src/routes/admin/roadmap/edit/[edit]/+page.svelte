<script>
  ...
  let roadmaps = getContext('product_notes')
	...
	const handleSubmit = async () => {
		...
		asyncTryCatchToast(
			...
			},
			{
				...
				redirect: `/admin/roadmap/read/${editID}`
			}
		);
	};
	...
	onMount(() => {
      ...
      status:$status,
			editable: true,
			data: $roadmaps.find((e) => e.id == editID)
		});
	});
</script>
...
{:else}
	...
	>
		<div class="flex flex-row gap-1 text-[14px] leading-5 font-medium items-center">
			<a href="/admin/roadmaps/browse" class="text-gray-3">Roadmap</a>
	...
	<div class:hidden={!cancel} class=" absolute top-0 left-0 m-auto w-full h-screen bg-black-3 z-50">
		...
		>
			...
			<div class="flex flex-row gap-3 text-black font-semibold items-center w-full">
				...
				>
				<a
					href="/admin/roadmap/read/{editID}"
