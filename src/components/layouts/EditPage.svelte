<script>
	...
	const getBrowseURL = () => {
		return currentPageRoute.split('edit')[0] + 'read' + currentPageRoute.split('edit')[1];
	};
...
>
	...
	<div class="flex flex-row justify-between items-start w-full gap-20">
		...
		<div class="flex flex-row gap-3 text-[13px] font-semibold leading-normal w-full justify-end">
			...
			<button
				type="submit"
				class="flex flex-row justify-center gap-2 items-center py-3 px-2 w-full max-w-[115px] max-h-[90px] bg-yellow rounded-md"
			>
				{#if loading}
					<img src="/images/icons/save.svg" alt="save" /> Updating...
				{:else}
					<img src="/images/icons/save.svg" alt="save" /> Update
				{/if}
			</button>
		</div>
	</div>
